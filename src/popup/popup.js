...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Clear All Leads (Fresh Start)
    document.getElementById('clear-btn').addEventListener('click', () => {
        if (!confirm('Are you sure? This will delete ALL extracted leads for a fresh start.')) return;

        chrome.runtime.sendMessage({ type: 'CLEAR_LEADS' }, (response) => {
            if (response && response.success) {
                alert('✅ All leads cleared! You can now start a fresh extraction.');
                updateUI();
            } else {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0]) {
                chrome.scripting.executeScript({
                    target: { tabId: tabs[0].id },
                    func: () => {
                        window.postMessage({ type: 'STOP_AUTO_SCRAPE' }, '*');
                    }
                });
            }
        });
